...
public:
    // Constructors
    Cube(const glm::mat4 &pctm, const ScenePrimitive &pprimitive);
    Cube();
...
private:
    ...
    void make_face(std::vector<GLfloat> &data, glm::vec3 top_left, glm::vec3 top_right, glm::vec3 bottom_left, glm::vec3 bottom_right, int shape_parameter_1);

    // Function to retrieve number of triangles for this class
    int get_triangles() override;

    // Fields unique to the Cube class

    // VAO shared across all instances of Cubes
    static GLuint m_vao;
    // VBO shared across all instances of Cubes
    static GLuint m_vbo;
    // Number of triangles shared across all instances of Cubes
